...
public class ProgramController {
	...
	@Autowired
	public ProgramController(AuthService authService, ProgramService programService, ChannelService channelService) {
	...
	public @ResponseBody AwsS3Post thumbUpload(@PathVariable long channelId, @PathVariable long programId, Model model, HttpServletRequest req) {
	...
	public @ResponseBody AwsS3Post fileUpload(@PathVariable long channelId, Model model, HttpServletRequest req) {
		...
		Properties pro = AwsLib.getAwsCredentials();
		String accessKey = pro.getProperty("accessKey");	    
		String redirectUrl = "http://" + req.getServerName() + ":" + req.getServerPort() + "/aws/contentDrop";		
		String contentKey = KeyFactory.keyToString(program.getKey());
	...
	public String editForm(@PathVariable long programId, Model model) {
		MsoProgram program = programService.findById(programId);
		MsoChannel channel = channelService.findById(program.getChannelKey().getId());
		String bucketName = "com-nnaws-showthumb";
		String uploadUrl = "http://" + bucketName + ".s3.amazonaws.com/";
	...
	public @ResponseBody String editSubmit(@PathVariable long programId, HttpServletRequest req, SessionStatus status) {
	...
	public String clips(@PathVariable long channelId, Model model, HttpSession session) {	
		List<MsoProgram> onair = programService.findByChannelIdAndIsPublic(channelId, true);
		List<MsoProgram> offair = programService.findByChannelIdAndIsPublic(channelId, false);
	...
	public ResponseEntity<String> onoff(@PathVariable String programIds, HttpServletRequest req) {
	...
	public String list(@PathVariable String cIdStr, Model model, HttpSession session) {
		...
		if (cIdStr.equals("*")) {
			...
			if (!exists) {channelId = channels.get(0).getId();}			
		}		
		List<MsoProgram> onair = programService.findByChannelIdAndIsPublic(channelId, true);
		List<MsoProgram> offair = programService.findByChannelIdAndIsPublic(channelId, false);
